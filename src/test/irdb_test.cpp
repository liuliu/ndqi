...
int main()
{
	...
	while ((entry = readdir(dir)) != NULL)
	{
		...
		if (S_ISREG(statinfo.st_mode))
		{
			...
			CvMat* irm = nqirnew(small, cvMSERParams(5, 60, cvRound(.2*small->width*small->height), .25, .2));
			if (irm != NULL)
			{
				//CvMat* ire = nqbweplr(irm);
				//printf("%d => %d\n", irm->rows, ire->rows);
				//cvReleaseMat(&irm);
				//if (ire != NULL)
				//{
					nqbwdbput(bwdb, entry->d_name, irm);
	...
	CvMat* irm = nqirnew(tsmall, cvMSERParams(5, 60, cvRound(.2*tsmall->width*tsmall->height), .25, .2));
	//CvMat* ire = nqbweplr(irm);
	...
	int k = nqbwdblike(bwdb, irm, kstr, 10, NQBW_LIKE_BEST_MATCH_SCORE, 100, 1, likeness);
